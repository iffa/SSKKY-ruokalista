...
import email.crappy.ssao.ruoka.recycler.SnappingLinearLayoutManager;
import email.crappy.ssao.ruoka.recycler.WeekAdapter;
...
public class FoodFragment extends MvpLceViewStateFragmentFixed<RelativeLayout, List<Ruoka>, FoodView, FoodPresenter> implements FoodView {
    ...
    public void goToCurrentWeek() {
        for (Ruoka ruoka : data) {
            ...
            if (DateUtil.isCurrentWeek(week)) {
                foodListView.scrollToPosition(data.indexOf(ruoka));
                break;
            }
        }
    }
    ...
    public void goToCurrentWeekSmooth() {
        for (Ruoka ruoka : data) {
            String week = ruoka.getTitle().split("\\s+")[1];
            if (DateUtil.isCurrentWeek(week)) {
                foodListView.smoothScrollToPosition(data.indexOf(ruoka));
                break;
            }
        }
    }
